...
const READING_TIME = 7; // seconds required to earn credits
...
const EmailInbox = () => {
  ...
  const [reading, setReading] = useState(null); // currently open email
  const [timer, setTimer] = useState(0);
  const [credited, setCredited] = useState([]); // ids already credited this session
  ...
  const fetchEmails = useCallback(async () => {
    setLoading(true);
    // Fetch campaigns from other members, newest first, excluding own
  ...
  const openEmail = (email) => {
    if (credited.includes(email.id)) return; // already earned for this one
    setReading(email);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.85)' }}>
          <div className="w-full max-w-2xl rounded-2xl overflow-hidden" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
            ...
            <div className="px-5 py-4" style={{ borderBottom: '1px solid var(--border)', background: 'var(--bg-secondary)' }}>
              <div className="flex items-center justify-between mb-2">
                <p className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>{reading.subject}</p>
                {timer > 0 ? (
