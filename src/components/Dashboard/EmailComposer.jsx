  ...
  const generateEmail = async () => {
    ...
    try {
      const response = await fetch('/.netlify/functions/generate-email', {
        ...
        body: JSON.stringify({ category: selectedCategory, tone: selectedTone, generationCount }),
      });
      if (!response.ok) {
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        <div className="card p-5 space-y-5">
          ...
          <div className="rounded-xl overflow-hidden" style={{ border: '1px solid rgba(0,212,120,0.25)', background: 'rgba(0,212,120,0.03)' }}>
            ...
            <div className="p-4 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {TONES.map((tone) => (
                    <button key={tone.value} type="button" onClick={() => setSelectedTone(tone.value)}
                      ...
                      style={{ background: selectedTone === tone.value ? 'rgba(0,212,120,0.12)' : 'var(--bg-secondary)', border: '1px solid ' + (selectedTone === tone.value ? 'rgba(0,212,120,0.5)' : 'var(--border)') }}>
                      <span className="text-sm font-semibold block" style={{ color: selectedTone === tone.value ? 'var(--brand-green)' : 'var(--text-primary)' }}>{tone.emoji} {tone.label}</span>
                      <span className="text-xs" style={{ color: 'var(--text-muted)' }}>{tone.desc}</span>
                    </button>
                  ))}
                </div>
              </div>
              <button type="button" onClick={generateEmail} disabled={isGenerating}
                className="w-full flex items-center justify-center gap-2 py-3 rounded-lg font-bold text-sm transition-all"
                style={{ background: isGenerating ? 'transparent' : 'linear-gradient(135deg, #00d478, #00ad60)', color: isGenerating ? 'var(--brand-green)' : '#0a0e1a', border: isGenerating ? '1px solid rgba(0,212,120,0.3)' : 'none', fontFamily: 'Syne' }}>
                {isGenerating ? (
                  <><div className="w-4 h-4 rounded-full border-2 animate-spin" style={{ borderColor: 'var(--brand-green)', borderTopColor: 'transparent' }} />Writing your {selectedCategory} email...</>
                ) : generationCount > 0 ? (
              ...
              {showVariations && subjectVariations.length > 0 && (
                <div className="rounded-lg overflow-hidden" style={{ border: '1px solid var(--border)' }}>
                  <div className="flex items-center justify-between px-3 py-2" style={{ background: 'var(--bg-secondary)', borderBottom: '1px solid var(--border)' }}>
                    ...
                    <button type="button" onClick={() => setShowVariations(false)} style={{ color: 'var(--text-muted)' }}><ChevronDown size={14} /></button>
                  </div>
                  {subjectVariations.map((s, i) => (
                    <button key={i} type="button" onClick={() => pickSubjectVariation(s)}
                      className="w-full text-left px-3 py-2.5 text-sm transition-colors"
                      style={{ borderBottom: i < subjectVariations.length - 1 ? '1px solid var(--border)' : 'none', color: generatedSubject === s ? 'var(--brand-green)' : 'var(--text-primary)', background: generatedSubject === s ? 'rgba(0,212,120,0.08)' : 'var(--bg-card)' }}>
              ...
              {generationCount > 0 && !showVariations && (
                <button type="button" onClick={() => setShowVariations(true)} className="text-xs flex items-center gap-1 hover:opacity-80" style={{ color: 'var(--brand-green)' }}>
          ...
          <div>
            ...
            <input {...register('subject', { required: 'Subject is required', minLength: { value: 5, message: 'Min 5 characters' }, maxLength: { value: 200, message: 'Max 200 characters' } })}
              placeholder="Use AI above to generate, or write your own subject line..." className="input" />
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              {generatedBody && <span className="text-xs flex items-center gap-1" style={{ color: 'var(--brand-green)' }}><Sparkles size={10} /> AI generated — feel free to edit</span>}
            </div>
            <textarea {...register('message', { required: 'Message is required', minLength: { value: 50, message: 'Min 50 characters' } })}
              rows={12} placeholder="Click the AI generator above to auto-populate..." className="input resize-y" />
            {errors.message && <p className="text-xs mt-1" style={{ color: '#f87171' }}>{errors.message.message}</p>}
            <p className="text-xs mt-1" style={{ color: 'var(--text-muted)' }}>💡 Replace <strong>[YOUR LINK HERE]</strong> with your actual affiliate or offer link before sending.</p>
          </div>
          <div>
            <label className="label">Recipients: <strong style={{ color: 'var(--brand-green)' }}>{parseInt(recipientCount || 0).toLocaleString()}</strong></label>
            <input {...register('recipientCount', { required: true, min: { value: 100, message: 'Minimum 100 recipients' }, max: { value: maxSendable, message: 'Maximum ' + maxSendable.toLocaleString() + ' recipients' } })}
              type="range" min={100} max={maxSendable || 100} step={100} className="w-full mt-2" style={{ accentColor: 'var(--brand-green)' }} />
            <div className="flex justify-between text-xs mt-1" style={{ color: 'var(--text-muted)' }}>
              <span>100</span><span>{maxSendable.toLocaleString()}</span>
            </div>
