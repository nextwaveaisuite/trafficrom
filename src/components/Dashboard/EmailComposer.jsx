...
import {
  ...
  Sparkles, RefreshCw, ChevronDown, Zap, User
} from 'lucide-react';
...
const EmailComposer = () => {
  ...
  // Insert [FIRST_NAME] tag at cursor position in body or subject
  const insertFirstName = (field) => {
    if (field === 'body') {
      ...
      const updated = current.slice(0, start) + '[FIRST_NAME]' + current.slice(end);
      setValue('message', updated);
      setTimeout(() => { ta.selectionStart = ta.selectionEnd = start + 12; ta.focus(); }, 0);
    } else {
      const current = getValues('subject') || '';
      setValue('subject', current + '[FIRST_NAME]');
    }
  };
        ...
        <div className="card p-5 space-y-5">
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <label className="label" style={{ marginBottom: 0 }}>Email subject *</label>
              <button type="button" onClick={() => insertFirstName('subject')}
                className="flex items-center gap-1 text-xs px-2 py-1 rounded-lg transition-all"
                style={{ background: 'rgba(0,212,120,0.1)', color: 'var(--brand-green)', border: '1px solid rgba(0,212,120,0.2)' }}>
                <User size={10} /> Insert [FIRST_NAME]
              </button>
            </div>
            <input
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <div className="flex items-center gap-2">
                ...
                <button type="button" onClick={() => insertFirstName('body')}
                  className="flex items-center gap-1 text-xs px-2 py-1 rounded-lg transition-all"
                  style={{ background: 'rgba(0,212,120,0.1)', color: 'var(--brand-green)', border: '1px solid rgba(0,212,120,0.2)' }}>
                  <User size={10} /> Insert [FIRST_NAME]
                </button>
            ...
            <textarea
              ...
              ref={(e) => { textareaRef.current = e; register('message').ref(e); }}
              rows={12}
              placeholder="Click the AI generator above to auto-populate, or write your own email here..."
              className="input resize-y"
            />
            {errors.message && <p className="text-xs mt-1" style={{ color: '#f87171' }}>{errors.message.message}</p>}
            <div className="flex items-start justify-between mt-1 gap-2">
              <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
                💡 Replace <strong>[YOUR LINK HERE]</strong> with your affiliate link before sending.
              </p>
              <p className="text-xs shrink-0" style={{ color: 'var(--text-muted)' }}>
                Use <strong style={{ color: 'var(--brand-green)' }}>[FIRST_NAME]</strong> to personalise with each reader's name.
              </p>
            </div>
          </div>
          ...
          <div>
            ...
            <input
              {...register('recipientCount', { required: true, min: { value: 100, message: 'Minimum 100 recipients' }, max: { value: maxSendable, message: 'Maximum ' + maxSendable.toLocaleString() + ' recipients' } })}
              type="range" min={100} max={maxSendable || 100} step={100} className="w-full mt-2"
