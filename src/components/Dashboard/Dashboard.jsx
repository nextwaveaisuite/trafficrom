...
import { Send, Mail, TrendingUp, Coins, ChevronRight, Gift, ArrowRight } from 'lucide-react';
...
const Skeleton = ({ w, h, rounded = 8 }) => (
  <div style={{ width: w, height: h, borderRadius: rounded, background: '#1e2840', animation: 'pulse 1.5s ease-in-out infinite' }} />
);
...
const StatCard = ({ icon: Icon, label, value, sub, accent, loading }) => (
  <div className="card p-5">
    ...
    {loading ? (
      <>
        <Skeleton w="60%" h={28} />
        <div style={{ marginTop: 6 }}><Skeleton w="80%" h={14} /></div>
      </>
    ) : (
      <>
...
const Dashboard = () => {
  ...
  useEffect(() => {
    if (!user) return;
    // Small delay so dashboard layout renders first
    const t = setTimeout(() => {
      supabase
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto animate-fade-in">
      ...
      <div className="mb-6">
        {profileLoading ? (
          <><Skeleton w={240} h={28} /><div style={{ marginTop: 8 }}><Skeleton w={160} h={14} /></div></>
        ) : (
          <>
      ...
      {/* Announcements — shows auto-promo codes and platform notices */}
      <Announcements />

      {/* Stats */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <StatCard icon={Coins} label="Credits balance" value={profile?.credits?.toLocaleString() || '0'} sub="Earn more by reading emails" accent loading={profileLoading} />
        <StatCard icon={Send} label="Emails sent today" value={profile?.emails_sent_today || 0} sub={`${Math.max(0, emailsRemaining)} sends remaining`} loading={profileLoading} />
        <StatCard icon={Mail} label="Total campaigns" value={profile?.total_emails_sent || 0} loading={profileLoading} />
        <StatCard icon={TrendingUp} label="Plan" value={tier.charAt(0).toUpperCase() + tier.slice(1)} sub={tier !== 'pro' ? 'Upgrade for more sends' : '✓ Max tier'} loading={profileLoading} />
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-4 mb-6">
        ...
        <div className="card p-5">
          ...
          <Link to="/dashboard/inbox" className="btn-outline inline-flex items-center gap-2 text-sm py-2 px-4">
            <Coins size={14} /> Read & Earn
          </Link>
        </div>
      </div>

      {/* Banner ad rotation */}
      <BannerRotator size="leaderboard" className="mb-6" />
