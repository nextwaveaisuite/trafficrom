...
const Leaderboard = () => {
  ...
  const fetchLeaderboards = async () => {
    ...
    (referrersRes.data || []).forEach(r => {
      ...
      if (!refCounts[id]) refCounts[id] = { count: 0, profile: r.profiles };
      refCounts[id].count++;
    });
    ...
    (mailersRes.data || []).forEach(r => {
      const id = r.user_id;
      if (!mailCounts[id]) mailCounts[id] = { count: 0, profile: r.profiles };
      mailCounts[id].count++;
    });
    ...
    (readersRes.data || []).forEach(r => {
      const id = r.user_id;
      if (!readCounts[id]) readCounts[id] = { total: 0, profile: r.profiles };
      readCounts[id].total += r.amount;
    });
        ...
        <div className="grid grid-cols-3 gap-3">
          {[1,2,3].map(rank => (
            <div key={rank} className="text-center p-3 rounded-lg" style={{ background: 'rgba(0,0,0,0.2)' }}>
              <RankBadge rank={rank} />
              <p className="text-lg font-bold mt-1" style={{ fontFamily: 'Syne', color: rank === 1 ? '#fbbf24' : rank === 2 ? '#94a3b8' : '#cd7f32' }}>
                {prizes[rank - 1].toLocaleString()}
              </p>
          ...
          return (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              ...
              style={{
                ...
                border: '1px solid ' + (activeTab === tab.id ? tab.color + '50' : 'var(--border)'),
              }}>
